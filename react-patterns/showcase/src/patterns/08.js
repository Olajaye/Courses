...
const Usage = () => {
  const {clapState, getTogglerProps, getCounterProps, reset }= useClapState(USER_INITIAL_STATE)
  const { count, countTotal, isClicked } = clapState;
  ...
  const [uploadeReset, setUpdatedReset] = useState(false)
  useEffectAfterMount(() => {
    setUpdatedReset(true)
    const id = setTimeout(() => {
      setUpdatedReset(false)
  ...
  return (
    ...
    >
      ...
      <section>
        ...
        <pre>{uploadeReset? "Uploading": "" }</pre>
      </section>
      </>
  );;
};
